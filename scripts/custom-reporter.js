...
class CustomReporter {
  constructor(options = {}) {
    ...
    this.generateOnEnd = options.generateOnEnd !== false;
    this.tests = [];
  }

  onBegin(config, suite) {
    this.config = config;
    console.log('CustomReporter: 开始收集测试结果...');
  }
  ...
  onTestEnd(test, result) {
    this.tests.push({
      ...
      suitePath: test.parent ? test.parent.titlePath() : []
    });
  }

  onEnd(result) {
    console.log(`CustomReporter: 收集完成，共 ${this.tests.length} 个测试用例`);
  ...
  generateReport(result) {
    console.log('CustomReporter: 开始生成自定义报告...');
    ...
    // 读取 Playwright 的 report.json
    const reportJsonPath = path.join(reportDir, 'report.json');
    let data = { suites: [] };
    
    if (fs.existsSync(reportJsonPath)) {
      try {
        data = JSON.parse(fs.readFileSync(reportJsonPath, 'utf-8'));
      } catch (err) {
        console.warn('无法读取 report.json，使用收集的测试数据:', err.message);
      }
    }

    // 复制附件
    const attachments = this.copyAttachments(reportDir);

    // 生成 HTML
    this.generateHtml(data, attachments, reportDir);
    
    console.log('CustomReporter: 报告生成完成！');
  }

  copyAttachments(reportDir) {
    ...
    testResultsDirs.forEach(resultsDir => {
      ...
      this.walkDirectory(resultsPath, resultsDir, attachments);
    });
  ...
  walkDirectory(dir, baseDir, attachments) {
    ...
    items.forEach(item => {
      ...
      if (stat.isDirectory()) {
        this.walkDirectory(itemPath, baseDir, attachments);
      } else {
        const ext = path.extname(item).toLowerCase();
        if (['.png', '.jpg', '.jpeg', '.gif', '.webp', '.webm', '.mp4', '.avi', '.txt', '.md', '.log'].includes(ext)) {
          const relPath = path.relative(path.join(process.cwd(), baseDir), itemPath);
  ...
  processSuite(suite, parentTitle = '', attachments) {
            ...
            spec.tests.forEach(test => {
              ...
              if (result && result.attachments && result.attachments.length > 0) {
                result.attachments.forEach((att, idx) => {
                  ...
                  let matchedAttachment = attachments.find(a => a.newName.includes(attFileName));
                  ...
                  if (!matchedAttachment && attFileName === 'test-failed-1.png') {
                    matchedAttachment = attachments.find(a => a.newName.includes('test_failed_1'));
                  }
                  if (!matchedAttachment && attFileName === 'error-context.md') {
                    matchedAttachment = attachments.find(a => a.newName.includes('error_context'));
                  }
    ...
    if (suite.suites && suite.suites.length > 0) {
      suite.suites.forEach(childSuite => {
        testHtml += this.processSuite(childSuite, currentTitle, attachments);
      });
    }
  ...
  countStats(suite) {
    ...
    if (suite.suites && suite.suites.length > 0) {
      suite.suites.forEach(childSuite => {
        const childStats = this.countStats(childSuite);
  ...
  generateHtml(data, attachments, reportDir) {
    let testHtml = '';
    data.suites.forEach(suite => {
      testHtml += this.processSuite(suite, '', attachments);
    });

    let stats = { total: 0, passed: 0, failed: 0, skipped: 0 };
    data.suites.forEach(suite => {
      const suiteStats = this.countStats(suite);
...
<body>
  <div class="container">
    ...
    <div class="summary">
      <div class="summary-item total">📊 总计: ${stats.total}</div>
      <div class="summary-item passed">✅ 通过: ${stats.passed}</div>
      <div class="summary-item failed">❌ 失败: ${stats.failed}</div>
      <div class="summary-item skipped">⏭️ 跳过: ${stats.skipped}</div>
    </div>
