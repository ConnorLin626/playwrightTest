...
let testData = _AccountReportsListPage.fetchTestData("CN_testData.json");
let transferSumReportName = "";


describe("Transfer Summary Report", async function () {
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.PaymentReport.SIT.loginCompanyId : testData.PaymentReport.UAT.loginCompanyId, SIT ? testData.PaymentReport.SIT.loginUserId : testData.PaymentReport.UAT.loginUserId, "P@ssword1A!"); });
    ...
    it("Create Transfer Summary Report", async function () {
        ...
        await process4InitCommonValue("TranSumReportName",_AccountReportsListPage.acctReportListPage.transferSumCreateButton).then(text => {
    ...
    it("View Report", async function () {
        await processGenerateReport(
            ...
            _AccountReportsListPage.acctReportListPage.transferSumActionGenerateButton,
...
async function loadCondition4Report(accountElement: TextInput) {
    ...
    await browser.wait(
        ExpectedConditions.stalenessOf(_AccountReportsListPage.paymentReportsPage.firstViewButton.element),_AccountReportsListPage.paymentReportsPage.firstViewButton.getTimeOut()
    );
    await browser.wait(
        ExpectedConditions.elementToBeClickable(accountElement.element),accountElement.getTimeOut()
    );
}
