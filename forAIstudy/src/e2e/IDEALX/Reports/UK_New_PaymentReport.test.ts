...
let testData = _AccountReportsListPage.fetchTestData("UK_testData.json");
...
describe("Payment Advice Report", async function () {
    ...
    it("Create Payment Advice Report", async function () {
        ...
        await process4InitCommonValue( "PayAdv", _AccountReportsListPage.acctReportListPage.payAdvCreateButton).then(text => { payAdvReportName = text; });
    ...
    it("Edit Payment Advice Report", async function () {
        await _AccountReportsListPage.acctReportListPage.filterInput.input(payAdvReportName);
