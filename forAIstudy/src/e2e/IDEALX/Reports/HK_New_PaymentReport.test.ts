...
let testData = _AccountReportsListPage.fetchTestData("HK_testData.json");
let payrollReportName = "";


describe("Payroll Report", async function () {
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.PaymentReport.SIT.loginCompanyId : testData.PaymentReport.UAT.loginCompanyId, SIT ? testData.PaymentReport.SIT.loginUserId : testData.PaymentReport.UAT.loginUserId, SIT ? testData.PaymentReport.SIT.pinId : testData.PaymentReport.UAT.pinId); });
    ...
    it("Create Payroll Report", async function () {
        ...
        await process4InitCommonValue("Payroll",_AccountReportsListPage.acctReportListPage.payrollCreateButton).then(text => {
    ...
    it("Edit Payroll Report", async function () {
        await _AccountReportsListPage.acctReportListPage.filterInput.input(payrollReportName);
...
describe("UX Payment Reports Deletion", async function () {
    ...
    it("Deletion Payroll Report", async function () {
        ...
        if ("" !== payrollReportName) {
            await deleteReport(payrollReportName,_AccountReportsListPage.acctReportListPage.payrollActionButton,_AccountReportsListPage.acctReportListPage.payrollActionDeleteButton);
        }
        else {
            await ListDelete(
...
async function process4OneOff(
    ...
    await Promise.all([
        ...
        await ensure(accountInView).textContains(SIT ? testData.PaymentReport.SIT.account: testData.PaymentReport.UAT.account),
...
async function process4InitCommonValue(prefixReportName: string,createBtn: Button) {
    ...
    await _AccountReportsListPage.createAccountReportsPage.searchInput.input(SIT ? testData.PaymentReport.SIT.accountNotInclude: testData.PaymentReport.UAT.accountNotInclude);
