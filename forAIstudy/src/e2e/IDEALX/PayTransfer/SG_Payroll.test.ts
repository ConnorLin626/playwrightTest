...
describe('SG_Payroll', async function () {
    this.retries(browser.params.caseRetryTimes);
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.Payroll.SIT.loginCompanyId : testData.Payroll.UAT.loginCompanyId, SIT ? testData.Payroll.SIT.loginUserId : testData.Payroll.UAT.loginUserId, "P@ssword1A!"); });
    ...
    it('Create Payroll with new payee', async function () {
        ...
        await _PaymentsPages.PayrollPage.amount.input(testData.Payroll.amountA1);
    ...
    it('Edit Payroll via Transfer Center', async function () {
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        await _PaymentsPages.TransferCentersPage.loadCondition();
        if (0 !== reference.trim().length) {
            await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaRef(reference);
        } else {
            await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaSearch("SG - Payroll", testData.status.PendingApproval);
        }
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
...
export async function checkViewPageAllField(isEdit :boolean) {
    await Promise.all([
        ...
        await ensure(_PaymentsPages.PayrollPage.amountView).textContains(isEdit? testData.Payroll.editamount : testData.Payroll.amountA1),
        ...
        await ensure(_PaymentsPages.PayrollPage.paymentSummaryValue).textContains(isEdit ? testData.Payroll.editamount : testData.Payroll.amountA1),
        ...
        await ensure(_PaymentsPages.PayrollPage.bankNameValue).textContains(SIT? testData.Payroll.SIT.bankName : testData.Payroll.UAT.bankName),
        ...
        await ensure(_PaymentsPages.PayrollPage.amountFirst).textContains(isEdit? testData.Payroll.editamount : testData.Payroll.amountA1),
        await ensure(_PaymentsPages.PayrollPage.purposeCodeValue).textContains(testData.Payroll.purposeCode),
