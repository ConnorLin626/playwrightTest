...
describe('UK_IntraCompanyTransfer_Approvals', async function () {
    this.retries(browser.params.caseRetryTimes);
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.IntraCompanyTransfer.SIT.loginCompanyId : testData.IntraCompanyTransfer.UAT.loginCompanyId, SIT ? testData.IntraCompanyTransfer.SIT.verifyUserId : testData.IntraCompanyTransfer.UAT.verifyUserId, "123123"); });
    const suitObject = this; beforeEach(async function(){process.env.currentTestTitle=this.currentTest.title;}); afterEach(async function () { handlerCase(suitObject, this, PROJECT_TYPE.IDEALX); });

    it('Verify an ICT Payment via My Verify', async function () {
        //'Verify an ICT Payment via My Verify
        await _ApprovalsPages.ApprovalPage.approvalMenu.click();
        await _ApprovalsPages.MyVerificationAndReleasePage.verifySingleTxnUnderList(_ApprovalsPages, verifyReference, testData.paymentType.ICT).then(reference => {
    ...
    it('Approve an ICT Payment via Transfer Center', async function () {
        ...
        await _PaymentsPages.IntraCompanyTransferPage.challengeResponse.input(testData.IntraCompanyTransfer.challengeCode);
        await _PaymentsPages.IntraCompanyTransferPage.approveBtn.click();
        await _PaymentsPages.IntraCompanyTransferPage.getDialogReferenceID().then(text => {
        ...
        await _PaymentsPages.IntraCompanyTransferPage.loadConditionForViewICTPaymentPage();
        await Promise.all([
            await ensure(_PaymentsPages.IntraCompanyTransferPage.transactionStatusValue).textContainsLessOne(testData.status.PendingVerification, testData.status.PendingRelease, testData.status.PartialApproved, testData.status.Approved, testData.status.BankRejected, testData.status.Completed)
        ]);
    });
    ...
    it('Release an ICT Payment via My Release', async function () {
        ...
        await _ApprovalsPages.MyVerificationAndReleasePage.releaseSingleTxnUnderList(_PaymentsPages, verifyReference, approvalReference, testData.paymentType.ICT).then(reference => {
