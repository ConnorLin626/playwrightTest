...
describe("AU_MPayroll Payment", async function () {
    ...
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.MPayrollPayment.SIT.loginCompanyId : testData.MPayrollPayment.UAT.loginCompanyId, SIT ? testData.MPayrollPayment.SIT.loginUserId : testData.MPayrollPayment.UAT.loginUserId, "P@ssword123"); });
    ...
    it('Edit a MPayroll Payment ', async function () {
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        await _PaymentsPages.TransferCentersPage.loadCondition();
        console.log("Mpayrollreference2:"+reference2);
        ...
        await _PaymentsPages.PayrollPage.newPayeeNickname.input(testData.MPayrollPayment.newPayeeNickname);
        await _PaymentsPages.PayrollPage.payeeBankID.input(SIT ? testData.MPayrollPayment.SIT.payeeBankIDEdit : testData.MPayrollPayment.UAT.payeeBankIDEdit);
        ...
        console.log("Mpayrollref:"+reference);
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaRef(reference);
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
        ...
        await Promise.all([
            await ensure(_PaymentsPages.PayrollPage.fromAccountView).isNotEmpty(),
            await ensure(_PaymentsPages.PayrollPage.amountView).textContains(testData.MPayrollPayment.amountEdit),
    ...
    it("Create with Approval Now without M-Chanllenge", async function () {
        ...
        await _PaymentsPages.PayrollPage.newPayeeAcctNo.input(testData.MPayrollPayment.newPayeeAcctNumber);
        await _PaymentsPages.PayrollPage.newPayeeButton.click();
        await _PaymentsPages.PayrollPage.amount.input(testData.MPayrollPayment.amountA2IX);
        await _PaymentsPages.PayrollPage.payeeRef.input(testData.MPayrollPayment.ReferenceForPayee);
        await _PaymentsPages.PayrollPage.showoptionaldetails.jsClick();
        await _PaymentsPages.PayrollPage.isBeneAdvising.jsClick();
        await _PaymentsPages.PayrollPage.emailIdO.input(testData.MPayrollPayment.emailIdO);
    ...
    it('Upload MPayroll Payment via Upload File', async function () {
        ...
        // upload File start
        await _FilesPages.uploadFilePage.fsUpload5(_FilesPages, "ALL - Universal File Format", lib_1.SIT ? testData.MPayrollPayment.SIT.fileName : testData.MPayrollPayment.UAT.fileName, testData.MPayrollPayment.approvalOptionByTxn).then(async (data) => {
            fileName = data;
        });
        // upload File end
    ...
    it('File Upload - Reject item for MPayroll Payment ', async function () {
        ...
        await _PaymentsPages.PayrollPage.reasonForRejection.input(testData.MPayrollPayment.rejectReason);
        await _PaymentsPages.PayrollPage.rejectDialogButton.click();
        await _PaymentsPages.PayrollPage.dismissButton.jsClick();
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaRef(fileName);
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
        await Promise.all([
    ...
    //Add for R8.7 IEBAA-1174
    it('View transaction page Download Reject item', async function () {
        // go to Transfer Center
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        //await _PaymentsPages.TransferCentersPage.loadCondition();
        await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaRef(fileName);
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
        await _PaymentsPages.PayrollPage.viewRejectedTab.click();
        await ensure(_PaymentsPages.PayrollPage.exportCSV).isNotDisabled();
        await _PaymentsPages.PayrollPage.exportCSV.click();
        // await _PaymentsPages.PayrollPage.beware.jsClick();
        // await _PaymentsPages.PayrollPage.confirm.jsClick();
    ...
    it('Create with Save as Template', async function () {
        ...
        await _PaymentsPages.PayrollPage.newPayeeNickname.input(tempNewPayeeName);
        await _PaymentsPages.PayrollPage.enterDetailButton.click();
        await _PaymentsPages.PayrollPage.bsbCodeText.input(testData.MPayrollPayment.bsbCode);
        await _PaymentsPages.PayrollPage.newPayeeAcctNo.input(testData.MPayrollPayment.newPayeeAcctNumber);
        await _PaymentsPages.PayrollPage.newPayeeButton.click();
    ...
    it('Save MPayroll Payment successfully As Draft', async function () {
        ...
        await _PaymentsPages.PayrollPage.getDialogReferenceID().then(text => {
            ...
            console.log("AU_ManagePayrollRef1:" + paymentReference);
        });
        await _PaymentsPages.PayrollPage.dismissButton.click();
        console.log("AU_ManagePayrollRef12" + paymentReference);
    ...
    it('Create a MPayroll Payment for verify and release', async function () {
        ...
        await _PaymentsPages.PayrollPage.newPayeeAcctNo.input(testData.MPayrollPayment.newPayeeAcctNumber);
        await _PaymentsPages.PayrollPage.newPayeeButton.click();
        await _PaymentsPages.PayrollPage.amount.input(testData.MPayrollPayment.amountVerifyIX);
        await _PaymentsPages.PayrollPage.payeeRef.input(testData.MPayrollPayment.ReferenceForPayee);
        await _PaymentsPages.PayrollPage.showoptionaldetails.jsClick();
        await _PaymentsPages.PayrollPage.isBeneAdvising.jsClick();
        await _PaymentsPages.PayrollPage.emailIdO.input(testData.MPayrollPayment.emailIdO);
        ...
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
        await console.log("MPverifyRef:"+verifyReference);
        await Promise.all([
            await ensure(_PaymentsPages.PayrollPage.fromAccountView).textContains(SIT ? testData.MPayrollPayment.SIT.fromAccount : testData.MPayrollPayment.UAT.fromAccount),
...
describe('Verify And Release A MPayroll Payment', async function () {
    this.retries(browser.params.caseRetryTimes);
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.MPayrollPayment.SIT.loginCompanyId : testData.MPayrollPayment.UAT.loginCompanyId, SIT ? testData.MPayrollPayment.SIT.verifyUserId : testData.MPayrollPayment.UAT.verifyUserId, "P@ssword123"); });
    const suitObject = this; beforeEach(async function(){process.env.currentTestTitle=this.currentTest.title;}); afterEach(async function () { handlerCase(suitObject, this, PROJECT_TYPE.IDEALX); });

    it('Verify a MPayroll Payment via My Verify', async function () {
        await _ApprovalsPages.ApprovalPage.approvalMenu.click();
        await _ApprovalsPages.MyVerificationAndReleasePage.verifySingleTxnUnderList(_ApprovalsPages, verifyReference, testData.paymentType.AU_MPYRL).then(reference => {
