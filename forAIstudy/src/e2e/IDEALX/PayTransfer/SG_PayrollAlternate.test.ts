...
describe('SG_Payroll Alternate', async function () {
    ...
    before(async function () { await new NavigatePages().loginIdealx(SIT ? testData.Payroll02.SIT.loginCompanyId : testData.Payroll02.UAT.loginCompanyId, SIT ? testData.Payroll02.SIT.loginUserId : testData.Payroll02.UAT.loginUserId, "P@ssword1A!"); });
    ...
    it('Create Payroll Alternate with new payee', async function () {
        ...
        await _PaymentsPages.PayrollPage.payeeBankID.input(testData.Payroll02.payeeBankID);
    ...
    it('Verify Payroll Alternate via My Verify', async function () {
        ...
        await new NavigatePages().loginIdealx(SIT ? testData.Payroll02.SIT.loginCompanyId : testData.Payroll02.UAT.loginCompanyId, SIT ? testData.Payroll02.SIT.verifyUserId : testData.Payroll02.UAT.verifyUserId, "P@ssword123");
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        await _PaymentsPages.TransferCentersPage.goToViewPaymentPageViaRef(reference3);
        await _PaymentsPages.PayrollPage.loadConditionForViewPaymentPage();
