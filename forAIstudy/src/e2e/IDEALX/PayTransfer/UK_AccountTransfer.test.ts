...
describe("UK Account Transfer Payment", async function () {
    ...
    it('Upload Account Transfer via Files to File Upload to Upload Files', async function () {
        ...
        // upload File start
        await _FilesPages.uploadFilePage.fsUpload5(_FilesPages, "ALL - Universal File Format", lib_1.SIT ? testData.AccountTransfer.SIT.fileName : testData.AccountTransfer.UAT.fileName, testData.AccountTransfer.approvalOptionByTxn).then(async (data) => {
            fileName = data;
        });
        // upload File
...
describe('Verify And Release A Account Transfer', async function () {
    ...
    it('Verify a Account Transfer via My Verify', async function () {
        ...
        await _ApprovalsPages.MyVerificationAndReleasePage.verifySingleTxnUnderList(_ApprovalsPages, verifyReference, testData.paymentType.UK_ACT).then(reference => {
    ...
    it('Approve a Account Transfer via Transfer Center', async function () {
        await _PaymentsPages.AccountTransferPage.paymentMenu.click();
        ...
        await Promise.all([
            await ensure(_PaymentsPages.AccountTransferPage.actStatusValue).textContainsLessOne(testData.status.PendingApproval, testData.status.Approved, testData.status.Completed, testData.status.BankRejected, testData.status.PendingRelease)
        ]);
    });
