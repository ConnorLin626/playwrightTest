...
describe('MO_TelegraphicTransfer', async function () {
    ...
    it('Create a TT Payment with new Payee', async function () {
        await _PaymentsPages.openMenu(Menu.Payments.MOPaymentLocalOverseasPayee);
        ...
        await _PaymentsPages.PaymentLocalOverseasPayeePage.payeeBankID.select(testData.TelegraphicTransfer.payeeBankID);
    ...
    it('Upload TT via Files->File Upload->Upload Files', async function () {
        // upload File start
        await _FilesPages.uploadFilePage.fsUpload(_FilesPages, testData.TelegraphicTransfer.fileType, testData.TelegraphicTransfer.fileFormat, lib_1.SIT ? testData.TelegraphicTransfer.fileName : testData.TelegraphicTransfer.fileName, testData.TelegraphicTransfer.approvalOptionByTxn).then(async (data) => {
            fileName = data;
        });
        // upload File end
        ...
        await Promise.all([
            await ensure(_PaymentsPages.PaymentLocalOverseasPayeePage.fromAccountValue).textContains(SIT ? testData.TelegraphicTransfer.SIT.fromAccount : testData.TelegraphicTransfer.UAT.fromAccount),
            await ensure(_PaymentsPages.PaymentLocalOverseasPayeePage.amountValue).textContains(testData.TelegraphicTransfer.amountUpload),
            await ensure(_PaymentsPages.PaymentLocalOverseasPayeePage.toNewPayeeNameValue).textContains(testData.TelegraphicTransfer.newPayeeName),
            await ensure(_PaymentsPages.PaymentLocalOverseasPayeePage.transactionStatus).textIs(testData.status.PendingApproval),
        ]);
    });
});
...
describe('MO_Telegraphic Transfer_Approvals', async function () {
    ...
    it('Verify a TT Payment via My Verify', async function () {
        ...
        await _ApprovalsPages.myVerificationAndReleasePage.verifySingleTxnUnderList(_PaymentsPages, reference3, 'MO - Telegraphic Transfer').then(reference => {
    ...
    it('Release a TT Payment via My Release', async function () {
        await _ApprovalsPages.myVerificationAndReleasePage.releaseSingleTxnUnderList(_PaymentsPages, verifyReference, approvalReference, "MO - Telegraphic Transfer").then(reference => {
