...
describe('MO_Account Transfer', async function () {
    ...
    it('Create an ACT Payment with ApprovalNow with Mchanllenge', async function () {
        ...
        await _PaymentsPages.AccountTransferPage.paymentDetail.input(testData.AccountTransfer.paymentDetail);
        await _PaymentsPages.AccountTransferPage.isBeneAdvising.jsClick();
        await _PaymentsPages.AccountTransferPage.faxTab.click();
        await _PaymentsPages.AccountTransferPage.faxCountryCode0.selectFirst();
        await _PaymentsPages.AccountTransferPage.faxAreaCode0.input(testData.AccountTransfer.faxAreaCode0);
        await _PaymentsPages.AccountTransferPage.faxNo0.input(testData.AccountTransfer.faxNo0);
        await _PaymentsPages.AccountTransferPage.message.input(testData.AccountTransfer.message);
        await _PaymentsPages.AccountTransferPage.isTransactionNote.jsClick();
        await _PaymentsPages.AccountTransferPage.transactionNote.input(testData.AccountTransfer.transactionNote);
        await _PaymentsPages.AccountTransferPage.nextButton.click();
    ...
    it('Upload Account Transfer via Files->File Upload->Upload Files', async function () {
        // upload File start
        await _FilesPages.uploadFilePage.fsUpload(_FilesPages, testData.AccountTransfer.fileType, testData.AccountTransfer.fileFormat, SIT ? testData.AccountTransfer.SIT.fileName : testData.AccountTransfer.UAT.fileName, testData.AccountTransfer.approvalOptionByTxn).then(async (data) => {
            fileName = data;
        });
        // upload File end
...
describe('MO_Account Transfer_Approvals', async function () {
    ...
    it('Verify an ACT Payment via My Verify', async function () {
        await _ApprovalsPages.myVerificationAndReleasePage.verifySingleTxnUnderList(_PaymentsPages, reference3, 'Account Transfer').then(reference => {
    ...
    it('Release an ACT Payment via My Release', async function () {
        await _ApprovalsPages.myVerificationAndReleasePage.releaseSingleTxnUnderList(_PaymentsPages, verifyReference, approvalReference, 'Account Transfer').then(reference => {
