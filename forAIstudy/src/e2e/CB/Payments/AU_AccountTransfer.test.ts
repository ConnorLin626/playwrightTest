...
describe("AU Account Transfer Payment", async function() {
	...
	it('Create a Account Transfer with new Payee', async function () {
		...
		await _PaymentsPages.AccountTransferPage.newPayeeCountry.select("AUSTRALIA");
	...
	it('Edit a Account Transfer ', async function () {
		...
		await _PaymentsPages.AccountTransferPage.amount.input(testData.BulkPayment.amountEdit);
		await _PaymentsPages.AccountTransferPage.newPayeeName.input(testData.BulkPayment.newPayeeNameEdit);
		await _PaymentsPages.AccountTransferPage.nextButton.click();
		await _PaymentsPages.AccountTransferPage.loadConditionForPrevewPage();
		await _PaymentsPages.AccountTransferPage.submitButton.click();
		await _PaymentsPages.AccountTransferPage.getInfoReferenceID().then(text => {
	...
	it('Upload Account Transfer via Files - File Upload - Upload Files', async function () {
		...
		await _FilesPages.filemanagement_UploadFile.fsUpload(_FilesPages, paymentType, SIT ? testData.AccountTransfer.SIT.fileName : testData.AccountTransfer.UAT.fileName, testData.AccountTransfer.approvalOptionByTxn).then(async (data) => {
			fileName = data;
		});
		// upload File end
