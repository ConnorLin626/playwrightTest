...
describe("UX OAT_BF Reports", async function() {
  ...
  it("Anchor Seller - Disbursement Report create", async function() {
    await _OATReportsListPage.openMenu(Menu.Reports.BFReports);
    await _OATReportsListPage.oatReportsListPage.loadConditionCommon();
    await process4InitCommonValue(
      "ASDR",
      _OATReportsListPage.oatReportsListPage.ASDRCreateButton
    ).then(text => {
      BFReportName = text;
    });
    await _OATReportsListPage.createOATBFReportsPage.fileName.input("ASDR" + moment());
    await _OATReportsListPage.createOATBFReportsPage.dailyInThe.select(
    ...
    await _OATReportsListPage.createOATBFReportsPage.password.input(
      testData.CreateAccountReport.password
    );
    await _OATReportsListPage.createOATBFReportsPage.confirmPassword.input(
      testData.CreateAccountReport.confirmPassword
    );
    await _OATReportsListPage.createOATBFReportsPage.remarks.input(
    ...
    await Promise.all([
      ...
      await ensure(_OATReportsListPage.oatReportsListPage.bfasdrViewAcct).textContains(
        SIT
          ? testData.CreateOATReport.SIT.account
          : testData.CreateOATReport.UAT.account
      ),
      await ensure(_OATReportsListPage.oatReportsListPage.bfasdrSendToValue).textContains(
        SIT
...
async function process4InitCommonValue(
  ...
  await _OATReportsListPage.createOATBFReportsPage.loadCondition();
  await _OATReportsListPage.createOATBFReportsPage.reportName.input(
